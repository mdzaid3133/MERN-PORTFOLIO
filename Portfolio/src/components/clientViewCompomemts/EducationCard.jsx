...
function EducationCard({ education }) {
  const {currentMode} = useSelector((store)=> store.mode)
    useEffect(()=>{
        ...
        return (
            <div data-aos="fade-up" className={`${currentMode === 'dark' ? ' bg-[#28262f] text-white' : ' text-gray-800 border-gray-800'} relative p-6  border  rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300`}>
