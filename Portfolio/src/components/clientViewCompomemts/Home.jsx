...
function Home() {
  ...
  return (
    <section className="my-component flex flex-wrap  justify-between px-5 md:px-32 md:py-[7.3rem] py-[3rem] gap-12 md:gap-0 ">
      <div className="text-center md:text-left md:w-1/2 w-full">
        ...
        <div>
          <div className='mb-14 flex flex-col'>
            <h1 data-aos="fade-right">HELLO,MY NAME IS</h1>
            <div data-aos="fade-right" className='bg-blue-500 w-20 h-1 mt-2 rounded-full hidden md:flex hover:w-36 transition ease-in duration-500'></div>
          </div>
          <h1 data-aos="fade-right" className="title md:text-6xl text-5xl font-bold capitalize">
            {homeData?.heading || 'Md. Zaid'}
          </h1>
          <h3 data-aos="fade-right" className="subTitle md:text-3xl text-xl font-medium mt-3 ">
            And I'm a <span className='text-blue-500'>{text}</span><Cursor className="text-blue-500" />
          </h3>
          <p data-aos="fade-right" className="md:text-lg text-sm md:mt-10 mt-5">
            {homeData?.summary || `Passionate Computer Science Undergrad,Competitive Programmer | MERN Stack Enthusiast`}
          </p>

          <div className="mt-6">
            <Button variant="outline" className=" rounded-full mr-4 bg-inherit hover:bg-blue-500 hover:text-white transition duration-200 ease-in">Hire Me</Button>
            <Button variant="outline" className=" border border-gray-400  py-1 bg-inherit bg-slate-50 text-gray-800 hover:bg-blue-500  hover:text-white transition duration-200 ease-in rounded-full"
      ...
      <div data-aos="zoom-in" className="md:w-1/2 w-full  flex justify-center">
        <div className="md:w-[400px] md:h-[400px] w-[300px] h-[300px] rounded-full overflow-hidden shadow-md shadow-white  border-4">
          <img src={homeData?.homeImage?.secure_url} className='rounded-full w-[100%] h-[100%]  ' />
        </div>
      </div>
    </section>
  );
